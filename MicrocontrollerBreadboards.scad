...
module single_board(x, y, board_specs)
{
    ...
    union()
    {
        ...
        difference()
        {
            ...
            pin_rows (x, y+pin_pitch, board_specs);
...
module eee_stud(x,y,sh,off=0,oil=2,maxid=3.2,ted=2.8+0.30,rmwt=0) {	
	...
	//difference() {
		...
		#union() {
			translate([x,y,hh]) cylinder(d=ophs,oil+ahd); // Inside Hole
			translate([x,y,st_h-th+off]) cylinder(d1=ophs,d2=oshd,th); // Inside Taper
		}			
	//}	
}
...
{
    translate([x,y,z])
        linear_extrude(height=th, convexity=4)
